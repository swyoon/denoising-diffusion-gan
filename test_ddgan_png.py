...
def sample_from_model(coefficients, generator, n_time, x_init, T, opt):
    ...
    with torch.no_grad():
        for i in reversed(range(n_time)):
            ...
            t_time = t
            if args.noz:
                latent_z = torch.ones(x.size(0), opt.nz, device=x.device)#.to(x.device)
            else:
...
def sample_and_test(args):
    ...
    if args.dataset == 'cifar10':
        ...
        real_img_dir = 'cifar10_train_png'
    ...
    save_dir = "./generated_samples_png/{}/{}".format(args.dataset, args.exp)
        ...
        for i in range(iters_needed):
            with torch.no_grad():
                ...
                for j, x in enumerate(fake_sample):
                    ...
                    torchvision.utils.save_image(x, './generated_samples_png/{}/{}/{}.png'.format(args.dataset, args.exp index))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--noz', action='store_true', default=False)
